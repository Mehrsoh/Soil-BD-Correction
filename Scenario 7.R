...
# Developing a PTF considering environmental covariates (-soil types) for predicting BD based on the MARTHA dataset using Random Forest Model (RF) with cross validation 
...
for (i in 1:n) {
  ...
  set.seed(2024)
  rf.model <- train(
    x=train.set[,c(1,3:21,23:59)], # names(martha_clean[,c(1,3:21,23:59)])
    y=train.set$BD,
    method = "ranger",
    tuneGrid=t.grid,
  ...
  if (i == n) {
    ...
    saveRDS(final_model, file = "H:/Analysis/Scenarios/Final_RF_Model_scenario 7.rds")
  }
...
saveRDS(validation, file = "H:/Analysis/Scenarios/Validation_scenario 7.rds")
