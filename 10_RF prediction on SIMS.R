...
sims_clean$se <- as.vector(predict(RF.model$finalModel, sims_clean, type="se")$se)

# 7.1. Select final columns for report----
final_table <- sims_clean %>%
  select(Profile_ID, Layer_ID, TOP, BOTTOM, Average_depth,
         BD, BD.pred, q05, q95, se)

# 7.2. Save results ----
write.csv(final_table, "SIMS_corrected_BD.csv", row.names = FALSE)
saveRDS(final_table, "SIMS_corrected_BD.rds")
