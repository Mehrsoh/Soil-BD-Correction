...
for (i in 1:n) {
  ...
  # 7. Save the last model for summary ----
  if (i == n) {
    final_model <- model
    saveRDS(final_model, file = "H:/Analysis/final_stepwise_MLR_model.rds")
  }
  
  # 8. Validation----
  
  # 8.1 predict using the test set ====
  test.set$BD_pred <- predict(model, newdata = test.set)
  
  # 8.2 # Calculate error measures ====
...
# 9. Check the validation metrics ----
