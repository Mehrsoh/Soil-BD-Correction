...
# Developing a PTF considering environmental covariates (-soil types) + SOC for predicting BD based on the MARTHA dataset using Random Forest Model (RF) with cross validation 
...
for (i in 1:n) {
 # 7.1. Split into training and test sets ====
  ...
  # 7.2. Define tuning grid for RF model ====
  t.grid <- expand.grid(mtry=c(1, seq(5,50,by=5), 53),  # mtry= number of independant variables
                        splitrule="variance",
                        min.node.size=c(5))
  
  # 7.3. Train RF model using caret ====
  set.seed(2024)
  rf.model <- train(
    x=train.set[,c(1,3,8:21,23:59)], # names(martha_clean[,c(1,3,8:21,23:59)])
    y=train.set$BD,
    method = "ranger",
    tuneGrid=t.grid,
  ...
  # 7.6. Save final model only at last fold ====
  if (i == n) {
    final_model <- rf.model
    saveRDS(final_model, file = "H:/Analysis/Scenarios/Final_RF_Model_scenario 2.rds")
  }
...
# 8.  Model checking ----
# 8.1. Plot final model performance ====
plot(final_model)
summary(final_model$finalModel)

# 8.2. Summarize validation results ====
print(round(colMeans(validation[, -1], na.rm = TRUE), 4)) # Mean of metrics
print(round(apply(validation[, -1], 2, sd, na.rm = TRUE), 4)) # Mean of metrics

# 8.3. Save validation results ====
saveRDS(validation, file = "H:/Analysis/Scenarios/Validation_scenario 2.rds")
