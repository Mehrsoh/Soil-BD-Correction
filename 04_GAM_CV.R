...
# 11. Save final model on last fold
  if (i == n) {
    final_model <- model
    saveRDS(final_model, file = "H:/Analysis/final_GAM_model.rds") # Export model to file for reuse
  }
  
# 9. Predict on test set ----
  test.set$BD_pred <- predict(model, newdata = test.set) # Predict BD for test set using trained model
  
# 10. Convert to numeric safely
