...
# Developing a PTF considering environmental covariates + SOC + PH_H2O + Sand + Clay for predicting BD based on the MARTHA dataset using Random Forest Model (RF) with cross validation 
...
for (i in 1:n) {
  ...
  t.grid <- expand.grid(mtry=c(1, seq(5,55,by=5), 57),  # mtry= number of independant variables
                        splitrule="variance",
                        min.node.size=c(5))
  ...
  set.seed(2024)
  rf.model <- train(
    x=train.set[,c(1,3:4,6:59)], # names(martha_clean[,c(1,3:4,6:59)])
    y=train.set$BD,
    method = "ranger",
    tuneGrid=t.grid,
  ...
  if (i == n) {
    ...
    saveRDS(final_model, file = "H:/Analysis/Scenarios/Final_RF_Mode1_scenario 5.rds")
  }
...
saveRDS(validation, file = "H:/Analysis/Scenarios/Validation_scenario 5.rds")
